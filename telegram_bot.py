...
VISIT_PROMPT = """You are a Medical Visit Documentation Assistant for Dr. Shiv Shankar Sharma, a visiting nephrologist/intensivist in Indore, India.
...
[{
  ...
  "phone": "Patient phone or mobile number if visible anywhere, else null",
  ...
  "keyEvents": [{"type": "One of: Hemodialysis, ICU Admission/Transfer, Antibiotic Change, Critical Lab Result, Imaging Finding, Procedure", "detail": "Specific detail with values drug names doses"}],
  "routineOnly": true
}]

routineOnly=true only if zero key events. Return [] if no patient data found."""

LAB_PROMPT = """You are a Medical Lab Report Extractor for Dr. Shiv Shankar Sharma's nephrology practice.
...
- For investigation flow sheets with serial results, extract each date column separately

Return ONLY a valid JSON object. No markdown, no backticks.

{
  "patientName": "Patient name from sticker or sheet header",
  "patientId": "MRN or IP number if visible, null if absent",
  "hospital": "Hospital name if visible",
  "results": [{"date": "DD/MM/YYYY", "parameter": "Test name e.g. Creatinine", "value": "Numeric value", "unit": "Unit if visible", "flag": "H if high L if low C if critical N if normal"}],
  "imaging": [{"date": "DD/MM/YYYY", "modality": "USG/CT/MRI/Echo/CXR", "finding": "Finding as written"}],
...
def extract_visit(image_bytes):
    text = call_gemini(image_bytes, VISIT_PROMPT)
    return json.loads(text)


def extract_labs(image_bytes):
    text = call_gemini(image_bytes, LAB_PROMPT)
    return json.loads(text)
...
VISIT_HEADERS = [
    ...
    "Hospital", "Ward", "OPD/IPD", "Referred By", "Phone",
